...
applications and retransmission do not occur.
...
* Handle closures. The compiler can generate code to allocate memory on the
  heap to store the data that closures capture (especially in writers).
...
the *homed* memory pages like the current one does.

Existing allocations could also be removed (especially by rewriting closures).

## Performances of the TILE-Gx36

In the course of the development of this project, I noticed that the *TILE-Gx36*
was seriously slower than present-day *x86* microprocessors, even with all its
36 cores used efficiently.
...
* *7zip*, an advanced multi-threaded compression/decompression tool.
* *lz4*, a compression/decompression tool written for real-time application.
  The tool only use one single core.
...
relative the *TILE-Gx36* (i.e. a value of 5 means that the chip is 5 times 
faster than the *TILE-Gx36*):
