...
* **The overhead of context-switching, inter-process data sharing and system
  ...
  framework is also more CPU cache friendly, as temporal locality is high.
...
here shows how some *TCP* payload checksums can be pre-computed**.
...
Rather than doing that, I adapted to ones' complement sum a good old algorithmic
technique. The method can find the sum of integers in any contiguous sub-vector
of an integer vector in only two memory accesses.
...
This trick can also be used with ones' complement sums, provided some
adjustments because of sums being computed over two bytes. It is used to
precompute ones' complement sums of the files served by the web-server. The
memory space usage of the `S` vector is linear to the number of bytes it was
computed from (`O(n)`), and only two memory accesses to the vector are required
to compute any ones's complement sum.
