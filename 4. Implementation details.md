...
for (int i = 0; i < size / sizeof (uint16_t); i++) {
    sum += p[i];
    sum = (sum & 0xFFFF) + (sum >> 16); // Reports and sums the carry bit.
}
...
The reduce the cost of computing the checksum, the framework does not use the
algorithm presented in the preceeding paragraph. Instead, the checksum is
computed using a more efficient algorithm based on techniques and properties of
the ones' complement addition that are featured in [RFC1071].

Instead adding 16-bits and adding the carry bit at each iteration, I use a
64-bits integer as sum's accumulator to computes two 16-bits sums at the same
time. The 32 mosts significant bits of the 64-bits accumulator will accumulate
carry bits while the 32 least significant bits will accumulate two 16 bits sums:
...
// Sums the data, 32-bits a a time.
