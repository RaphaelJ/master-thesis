...
data-structure only contains the first two fields. Using or not chained buffers
do not significantly affect network throughput. However, it proves that the
network stack is able to smoothly work with different network buffer
representations.
...
connection should have access to the network buffers, through a cursor.

However, the network buffer could not be allocated at the time the `send` 
method is called:

* Because the TCP transmitting window is empty and the 
* Because the sending of the 
