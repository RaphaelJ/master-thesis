...
(incomming) packet queues are instancied. **Each network stack instance is
continuously polling its queue for a new incomming packet to process. A core is
...
timers ...) **are cached on the core that handle the connection**.
