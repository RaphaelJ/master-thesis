...
* To provide a way to directly work on the buffer's bytes, without copying data.
* To be simple to use.
* To have slicing operators. Slicing makes it passing sub-sections of a packet
  to the upper network layer easier.

The framework use a data structure named *cursors* to abstract network buffers.
Cursor can be used to read and write data. It is highly influenced by
*ByteStrings*, a I/O data structure designed by *Don Stewart* for the *Haskell*
