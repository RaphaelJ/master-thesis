...
Both checksums are computed the same way, using the standized *Internet
Checksum* algorithm.
...
used to denotate a 2 bytes (16 bits) word. Bytes are ones' complement added in
pairs and a zero byte is added to have an even number of bytes. To compute the
*Internet Checksum* from this ones' complement sum we have, we have to compute
its ones' complement. The ones' complement of a number is its binary negation
(the `NOT` binary operator).

The ones's complement addition (the `+'` operator) is the traditional addition,
but with the carry bit added to the resulting sum. As an example, here is the 4
bits ones' complement binary addition of 1111 and 1011:
...
for (int i = 0; i < size / 2; i++) {
    ...
    sum += sum >> 16; // Reports and sums the carry bit.
}
