...
> **Note**
> One might think that allocating the network buffers in the `send` call,
> before creating the segment, could solve this problem.
>
...
The function should be able to only write a sub-section of the data, and should 
be able to be called repeatedly on the same data (in case of a retransmission). 
This function is called a *writer* in the framework, and its usage with the 
`send` function is as follow:
...
connection.send(
    ...
    // 'offset' values if the stack splits the data in multiple segment).
...
The *TCP* layer keeps writers in a transmission queue until the data they
write have been acknowledged.
