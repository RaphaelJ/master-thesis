...
> **Note**
> One might think that allocating the network buffers while calling `send`,
...
sum of all 16-bit words on which it is applied. A virtual zero byte is added at
the end if the data contain an odd number of bytes.

For example, if we want to compute the checksum of the bytes `[a, b, c, d, e,
f, g]`, the ones' complement sum would be `[a, b] +' [c, d] +' [e, f] +' [g,
0]`, where `+'` is the ones' complement addition and the notation `[a, b]` is
used to denotate a 2 bytes (16-bits) word. Byte pairs are ones' complement added
and a zero byte is added to have an even number of bytes. To compute the
*Internet Checksum* from this ones' complement sum, we have to compute its
ones' complement. The ones' complement of a number is its binary negation
(the `NOT` binary operator). The checksum of our 7 bytes is thus
`NOT([a, b] +' [c, d] +' [e, f] +' [g, 0])`
...
the 16-bits ones' complement binary addition of 1111 1011 1101 1110 and
1000 1001 1111 0001:

![16-bits ones's complement addition](img/ones_complement_addition.png)

A simplified and naive computation of the *Internet Checksum* could be the
following one:
...
presented in the preceeding paragraph. Instead, the checksum is computed using
...
single 64-bits integer. The 32 mosts significant bits of the 64-bits accumulator
will accumulate carry-bits while the 32 least significant bits will accumulate
two 16-bits sums:
...
// the 32-bits integer are added to the second sum, while the last 2 bytes are
...
> **Note**
> The actual algorithm is slightly more complicated as this one assumes that the
> data hold on a multiple of 4 bytes. The actual algorithm has also to deal with
...
1. The ones' complement addition is commutative and associative. That's why you
   can compute two sums at a time.
2. The carry-bit addition being a simple addition, it can be deferred.
   Carries of the first sum are added to the second sum, while carries of the
   second sum are added to the carry-bit accumulator. Both will be added to the
   final sum, meaning that they will not be "lost".
